...
# 2. Model(Sequential)
model = Sequential()
model.add(Conv2D(128, (3,3), padding='same', activation='relu', input_shape=(8,8,1)))
...
modelCheckPoint = ModelCheckpoint(monitor='val_loss', mode='min', verbose=1,
                                   save_best_only=True,
                                   filepath='MCP/keras39_9_digits_MCP.hdf5')
...
model.save(path+'keras39_9_digits_save_model.h5') # 가중치 및 모델 세이브
...
Result(CNN)
accuarcy_score:  0.987037037037037

...
model.add(Dense(7, activation='softmax')) # 다중 분류
model.summary()
...
modelCheckPoint = ModelCheckpoint(monitor='val_loss', mode='min', verbose=1,
                                   save_best_only=True,
                                   filepath='MCP/keras39_10_fetch_gd_MCP.hdf5')
...
model.save(path+'keras39_10_fetch_gd_save_model.h5') # 가중치 및 모델 세이브
...
Result(CNN)
accuarcy_score:  0.8947585827060768
